...
const register = async (data: {
  ...
  sendEmail({
    ...
    logger.error('Verify email send failed:', err);
    console.error('EMAIL ERROR DETAILS:', JSON.stringify(err, null, 2)); // add this
  });
...
const verifyEmail = async (token: string) => {
  ...
  await prisma.user.update({
    where: { id: user.id },
    data: { emailVerifiedAt: new Date(), status: newStatus },
  });
};
...
const enable2FA = async (userId: string, token: string) => {
  ...
  if (!user || !user.twoFactorSecret)
    throw new BadRequestError("Setup 2FA first");
